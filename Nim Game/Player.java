...
public class Player {
    ...
    /** The constant integer representing minimum number of stick that can be on board before end game*/
	private static final int MIN_UNMARKED_STICKS = 1;
	...
	private static final String MSG_ASK_PLAYER = "Press 1 to display the board. Press 2 to make a move:";
    private static final String MSG_INVALID_RESPONSE = "Unsupported command";
    private static final String MSG_ASK_ROW = "Enter the row number:";
    private static final String MSG_ASK_LEFT = "Enter the index of the leftmost stick:";
    private static final String MSG_ASK_RIGHT = "Enter the index of the rightmost stick:";
    ...
    /**
     * Check if all sticks on a row are marked.
     **/
	private boolean isFullRow(Board board, int row){
	    for (int i=1; i<=board.getRowLength(row); i++){
            if(board.isStickUnmarked(row, i)){
	            return false;
            }
        }
        return true; // if all sticks on the row are marked
    }

    /**
     * Check if a move is valid.
     **/
    private boolean isValid(Board board, int row, int left, int right){
	...
	private Move produceRandomMove(Board board){
        ...
        int randRow = theRandom.nextInt(board.getNumberOfRows()) + 1; // find a valid row
		while (isFullRow(board, randRow)) {
            randRow = theRandom.nextInt(board.getNumberOfRows()) + 1;
        }

        int randLeft = theRandom.nextInt(board.getRowLength(randRow)) + 1;  // find a valid coordinate
		while (!board.isStickUnmarked(randRow,randLeft)) {
            randLeft = theRandom.nextInt(board.getRowLength(randRow)) + 1;
        }

		// find a valid length
        int randRight = theRandom.nextInt(board.getRowLength(randRow) - randLeft + 1) + randLeft;

        while (!isValid(board,randRow, randLeft, randRight)){
            randRight = theRandom.nextInt(board.getRowLength(randRow) - randLeft + 1) + randLeft;
	...
	private Move produceSmartMove(Board board){
        ...
        for (int i=1; i <= board.getNumberOfRows(); i++){ // Find the last row with unmarked sticks
        ...
        if (fullRows == board.getNumberOfRows() - 1 && board.getNumberOfUnmarkedSticks() > MIN_UNMARKED_STICKS){
            // check if he can take win
            for (int j = 1; j <= board.getRowLength(smartRow); j++) {
                if (isValid(board, smartRow, j, j + board.getNumberOfUnmarkedSticks() -
                        MIN_UNMARKED_STICKS - 1)) {
                    return new Move(smartRow, j, j + board.getNumberOfUnmarkedSticks() -
                            MIN_UNMARKED_STICKS -1);
                }
            }
        }
        // if he can't win now produce a random move
        return produceRandomMove(board);
	}
	...
	private Move produceHumanMove(Board board){
        while (true) {
            System.out.println(MSG_ASK_PLAYER);
            ...
            if (userInput == 1) {
                ...
                System.out.println(MSG_INVALID_RESPONSE);
            }
        }
        // asking to the player the move that he want to do
        System.out.println(MSG_ASK_ROW);
        int rowInput = scanner.nextInt();
        System.out.println(MSG_ASK_LEFT);
        int leftInput = scanner.nextInt();
        System.out.println(MSG_ASK_RIGHT);
